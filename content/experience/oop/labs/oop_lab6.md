...
public:
    ...
    Directory(const string& company) : company(company), size(0), capacity(5), entries(new Entry*[5]){}
    ...
    unsigned operator[](const string& entry_name) const {
        for (size_t i = 0; i < size; ++i){
            if(entries[i]->getName() == entry_name){
